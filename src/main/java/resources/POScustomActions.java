...
public class POScustomActions extends base {
	...
	public static String pointofsale(String memlastname, String cvvcode) {
		pageObjects.POSmodulePO po = new pageObjects.POSmodulePO(driver);

		po.getPOSmodulebtn().click();
	
		po.getmemberbarcode().click();
		po.getlastname().sendKeys(memlastname);
		
		po.getsearchbtn().click();
		po.getselectmemberbtn().click();
		po.getitemfieldbarcode().click();
		po.getinventorybtn().click();
		po.getitemsearch().click();
		po.getitemselect().click();
		
		po.getbillproceed().click();
