...
public class MembersReport extends base {
	...
	@Test(enabled = true, priority = 1, description = "Members Report_Member by Status")
	public void Members_MemberbyStatus() throws Exception {

		// Assert.assertEquals("Web Reports" ,report.getpageTitle().getText());
		jse.executeScript("arguments[0].click();", report.getMembersmodule());
		Thread.sleep(200);
		Assert.assertEquals("Members", report.getMembers_PageHeader().getText());

		jse.executeScript("arguments[0].click();", report.getSelect_MemberbyStatus());

		Assert.assertEquals("Member by Status", report.getverifyReportText().getText());

		jse.executeScript("arguments[0].click();", report.getViewReportforMemberStatus());
		...
		Assert.assertTrue(driver.findElement(By.xpath("//*[@id='Text7']/p[1]/span/span")).isDisplayed());
		System.out.println(
				"Report name and date:  " + driver.findElement(By.xpath("//*[@id='Text7']/p[1]/span/span")).getText());
